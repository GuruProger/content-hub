...
async def create_like_endpoint(like_in: LikeCreate, db: AsyncSession = Depends(db_helper.session_getter)):
    manager = LikeManager(db)
...
@router.delete("/{article_id:int}/{user_id:int}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_like_endpoint(article_id: int, user_id: int, db: AsyncSession = Depends(db_helper.session_getter)):
    manager = LikeManager(db)
...
async def list_likes_by_article_endpoint(article_id: int, db: AsyncSession = Depends(db_helper.session_getter)):
    manager = LikeManager(db)
    likes = await manager.list_likes_by_article(article_id)
    return likes

@router.get("/user/{user_id:int}", response_model=list[LikeOut])
async def list_likes_by_user_endpoint(user_id: int, db: AsyncSession = Depends(db_helper.session_getter)):
    manager = LikeManager(db)
