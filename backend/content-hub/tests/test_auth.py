...
async def test_users_me(async_client, setup_test_user: User):
    ...
    response = await async_client.get(
        "/jwt/users/me/", headers={"Authorization": f"Bearer {token}"}
    )
    assert response.status_code == 200
    response_data = response.json()
...
async def test_register_duplicate(async_client):
    ...
    ), f"Duplicate registration did not produce expected error: {response2.text}"
...
async def test_register_missing_field(async_client):
    payload = {"username": "test_register_missing", "password": "my_secure_password"}
    response = await async_client.post("/jwt/register/", json=payload)
    assert (
        response.status_code == 422
    ), f"Validation error was not triggered: {response.text}"
